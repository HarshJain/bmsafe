...
uint16 niveau_1 = 0;                  // 1 = configuration, 2 = status, 3 = commandes
uint16 niveau_2 = 0;

// strings du menu d'affichage des paramètres                                            
uint16 gSciDisplayNumModule;                       // ID du slave à afficher
uint16 gSciInterruptMode = 0;                      //0 = mode menu, 1 = mode assignation

uint8 gGuiBuffer[GUI_RX_BUFFER_SIZE];            //Un tampon contenant les bytes reçus du lien avec l'usager à traiter.
uint8 gGuiReadIndex = 0;
uint8 gGuiWriteIndex = 0;
uint8 gGuiBufferFull = 0;


uint16 printStatTime = 3600;	// Nombre de temps (en secondes) pendant lequel on envoi les données statistiques
uint16 timeFollow = 0; 		// Simple compteur pour pouvoir savoir combien de temps on a fait (ou cycle)
...
//
//*****************************************************************************
void sciByteReception(uint8 rcvByte)
{
   uint8 i;

   static char inputBuf[20] = "";
   static uint8 bufPos = 0;
   float inputParam = 0;
   static uint8 point = 0;
...
void sciGetCellsData(int16 volt[][N_CELL], int temp[][N_CELL], int numer_mod)
{
   ...
   uint16 cell = 0; // cellule pour laquelle on affiche des donnés   
...
void SCIprintStatus(void)
{
    ...
    SCIprintString("\n\rCourant moyen: ");
    junk = sprintf(buf, formatCurrMa, (float)gCurrentFlow_mA/1000.0);
    SCIprintString(buf);
   ...
   {	
      if(input == 0) {    //Root menu
         ...
         switch (input) {
            ...
            case 'i': //ignoreInterlockState
            niveau_2 = 18;
            junk = sprintf(buf,"%u", gParams.ignoreInterlockState);		
            break;
         }
   ...
   {													  
      ...
      switch (input)
      {
         ...
         case '2':  //Désactiver l'équilibration
            if(gFlags.equilibrating) {
               if(gParams.manualMode) {
                  //CAN0SendEquiCommand(0, 4500, CAN_BROADCAST_ID); //4500: valeur supérieure à la tension max de cell, donc équilibration cesse // Old CAN code
         ...
         case '5':  //Activer charge				
            if(gMode != CHARGE_MODE){
               if(gParams.manualMode){
				  ...
				  {
					SCIprintString("Impossible d'entrer en mode recharge lorsque les relais sont fermés.\n\n\n\r");
				  }
         ...
         case '6':  //Désactiver charge
            if(gMode == CHARGE_MODE) {
               if(gParams.manualMode) {
                  gFlags.charger_online = 0; // TODO : on ne devrait pas utiliser ce flag pour faire ça (ne marche pas de toute façon)
		...
		switch (niveau_2)
		{
			...
			case 8:
			      ...
			      junk = sprintf(buf,formatTemp, (float)iNewParam/10.0);  
					break;
			case 9:
			      uiNewParam = (uint16)(user_input*1000);
			...
			case 12:
			      ...
			      junk = sprintf(buf,formatVolt, (float)uiNewParam/1000.0); 
					break;
			case 13:
			      uliNewParam = (uint32)(user_input*1000);
         ...
         case 18:
			      ucNewParam = (uint8)user_input;
			      gParams.ignoreInterlockState = ucNewParam;
			      junk = sprintf(buf, "%u", ucNewParam);  
				   break;                
		}
...
// Menu status -> sélection du module à afficher dans le terminal
	...
	{
	    gSciDisplayNumModule = (uint16) user_input; //assignation du nombre de module
	...
	//Menu commandes
		...
		{
			case 1:
		        uiNewParam = (uint16)(user_input*1000);      //Balance threshold
                sprintf(buf, "Valeur entrée: %u mV\n\n\r", uiNewParam);
                SCIprintString(buf);
            
				if((uiNewParam < gParams.maxCellVoltage) && (uiNewParam > gParams.minCellVoltage)) {
				  
					gBalanceThreshold_mV = (int16)uiNewParam; // Update threshold voltage
